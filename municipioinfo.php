...
// PHPMaker 6 configuration for Table municipio
$municipio = NULL; // Initialize table object
...
class cmunicipio {
	...
	function cmunicipio() {
		$this->TableVar = "municipio";
		$this->TableName = "municipio";
		$this->SelectLimit = TRUE;
		$this->idMunicipio = new cField('municipio', 'x_idMunicipio', 'idMunicipio', "`idMunicipio`", 3, -1, FALSE);
		$this->fields['idMunicipio'] =& $this->idMunicipio;
		$this->idRegional = new cField('municipio', 'x_idRegional', 'idRegional', "`idRegional`", 3, -1, FALSE);
		$this->fields['idRegional'] =& $this->idRegional;
		$this->idDepartamento = new cField('municipio', 'x_idDepartamento', 'idDepartamento', "`idDepartamento`", 3, -1, FALSE);
		$this->fields['idDepartamento'] =& $this->idDepartamento;
		$this->municipio = new cField('municipio', 'x_municipio', 'municipio', "`municipio`", 200, -1, FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `municipio` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		...
		$SQL .= EW_DB_QUOTE_START . 'idMunicipio' . EW_DB_QUOTE_END . '=' .	ew_QuotedValue($rs['idMunicipio'], $this->idMunicipio->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idMunicipio->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idMunicipio@", ew_AdjustSql($this->idMunicipio->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("municipioview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "municipioadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("municipioedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("municipioadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("municipiodelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idMunicipio->CurrentValue)) {
			$sUrl .= "idMunicipio=" . urlencode($this->idMunicipio->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=municipio" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		$this->municipio->CssStyle = "";
		$this->municipio->CssClass = "";
