...
// Create page object
var mer_edit = new ew_Page("mer_edit");

// page properties
mer_edit.PageID = "edit"; // page ID
var EW_PAGE_ID = mer_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
mer_edit.ValidateForm = function(fobj) {
...
mer_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mer_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker">Editar TABLA: Mer<br><br>
<a href="<?php echo $mer->getReturnUrl() ?>">Volver atras</a></span></p>
<?php $mer_edit->ShowMessage() ?>
<form name="fmeredit" id="fmeredit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mer_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="mer">
...
<?php if ($mer->idMer->Visible) { // idMer ?>
	<tr<?php echo $mer->idMer->RowAttributes ?>>
		...
		<td<?php echo $mer->idMer->CellAttributes() ?>><span id="el_idMer">
<div<?php echo $mer->idMer->ViewAttributes() ?>><?php echo $mer->idMer->EditValue ?></div><input type="hidden" name="x_idMer" id="x_idMer" value="<?php echo ew_HtmlEncode($mer->idMer->CurrentValue) ?>">
</span><?php echo $mer->idMer->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php if ($mer->idDepartamento->Visible) { // idDepartamento ?>
	<tr<?php echo $mer->idDepartamento->RowAttributes ?>>
		...
		<td<?php echo $mer->idDepartamento->CellAttributes() ?>><span id="el_idDepartamento">
                                    <input type="hidden" name="x_idDepartamento_edit" id="x_idDepartamento_edit" value="<?php echo $proyecto->idDepartamento->CurrentValue ?>"/>
...
<?php if ($mer->idMunicipio->Visible) { // idMunicipio ?>
	<tr<?php echo $mer->idMunicipio->RowAttributes ?>>
		...
		<td<?php echo $mer->idMunicipio->CellAttributes() ?>><span id="el_idMunicipio">
                                    <input type="hidden" name="x_idMunicipio_edit" id="x_idMunicipio_edit" value="<?php echo $proyecto->idMunicipio->CurrentValue ?>"/>
...
<?php if ($mer->idComunidad->Visible) { // idComunidad ?>
	<tr<?php echo $mer->idComunidad->RowAttributes ?>>
		...
		<td<?php echo $mer->idComunidad->CellAttributes() ?>><span id="el_idComunidad">
                                    <input type="hidden" name="x_idComunidad_edit" id="x_idComunidad_edit" value="<?php echo $proyecto->idComunidad->CurrentValue ?>"/>
...
<input type="text" name="x_mer" id="x_mer" size="30" maxlength="255" value="<?php echo $mer->mer->EditValue ?>"<?php echo $mer->mer->EditAttributes() ?>>
...
<?php if ($mer->fechaInicio->Visible) { // fechaInicio ?>
	<tr<?php echo $mer->fechaInicio->RowAttributes ?>>
		...
		<td<?php echo $mer->fechaInicio->CellAttributes() ?>><span id="el_fechaInicio">
                                    <input type="text" name="x_fechaInicio" id="x_fechaInicio" value="<?php echo $mer->fechaInicio->EditValue ?>"<?php echo $mer->fechaInicio->EditAttributes() ?> onchange="document.fmeredit.x_gestion.value=gestion(this.value)">
...
class cmer_edit {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$mer->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate Form
			if (!$this->ValidateForm()) {
				$mer->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$this->RestoreFormValues();
			}
		} else {
			$mer->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($mer->idMer->CurrentValue == "")
			$this->Page_Terminate("merlist.php"); // Invalid key, return to list
		switch ($mer->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$mer->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$mer->RowType = EW_ROWTYPE_EDIT; // Render as edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($mer->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// idMer
			$mer->idMer->EditCustomAttributes = "";
			$mer->idMer->EditValue = $mer->idMer->CurrentValue;
			$mer->idMer->CssStyle = "";
			$mer->idMer->CssClass = "";
			$mer->idMer->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (!ew_CheckNumber($mer->longitudUTM->FormValue)) {
			if ($gsFormError <> "") $gsFormError .= "<br>";
			$gsFormError .= "N�mero de Punto Flotante Incorrecto - Longitud UTM";
		}
		if (!ew_CheckNumber($mer->latitudUTM->FormValue)) {
			if ($gsFormError <> "") $gsFormError .= "<br>";
			$gsFormError .= "N�mero de Punto Flotante Incorrecto - Latitud UTM";
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$bUpdateRow = $mer->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($mer->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
