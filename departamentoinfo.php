...
// PHPMaker 6 configuration for Table departamento
$departamento = NULL; // Initialize table object
...
class cdepartamento {
	...
	function cdepartamento() {
		...
		$this->idDepartamento = new cField('departamento', 'x_idDepartamento', 'idDepartamento', "`idDepartamento`", 3, -1, FALSE);
		$this->fields['idDepartamento'] =& $this->idDepartamento;
		$this->idRegional = new cField('departamento', 'x_idRegional', 'idRegional', "`idRegional`", 3, -1, FALSE);
		$this->fields['idRegional'] =& $this->idRegional;
		$this->departamento = new cField('departamento', 'x_departamento', 'departamento', "`departamento`", 200, -1, FALSE);
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `departamento` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `departamento` WHERE ";
		$SQL .= EW_DB_QUOTE_START . 'idDepartamento' . EW_DB_QUOTE_END . '=' .	ew_QuotedValue($rs['idDepartamento'], $this->idDepartamento->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@idDepartamento@", ew_AdjustSql($this->idDepartamento->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("departamentoview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "departamentoadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("departamentoedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("departamentoadd.php", $this->UrlParm());
	}
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=departamento" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function RenderListRow() {
		...
		$this->departamento->CssStyle = "";
		$this->departamento->CssClass = "";
