...
<?php include "indicador3info.php" ?>
...
// Define page object
$indicador3_list = new cindicador3_list();
$Page =& $indicador3_list;

// Page init processing
$indicador3_list->Page_Init();

// Page main processing
$indicador3_list->Page_Main();
?>
...
<table>
     <tr style="background: #667120; color:#ffffff; font-size: 15px">
         <th colspan="2">PROPOSITO I.3:</th><td colspan="16">Municipios que contratan anualmente los servicios de al menos una MERS. A&ntilde;o 2: 4; Fin de proyecto: 8</td>
     </tr>
</table>
...
// Page Class
//
class cindicador3_list {

	// Page ID
	var $PageID = 'list';

	// Table Name
	var $TableName = 'indicador3';

	// Page Object Name
	var $PageObjName = 'indicador3_list';
	...
	function PageUrl() {
		...
		global $indicador3;
		if ($indicador3->UseTokenInUrl) $PageUrl .= "t=" . $indicador3->TableVar . "&"; // add page token
		return $PageUrl;
	}
	...
	function IsPageRequest() {
		global $objForm, $indicador3;
		if ($indicador3->UseTokenInUrl) {

			//IsPageRequest = False
			if ($objForm)
				return ($indicador3->TableVar == $objForm->GetValue("t"));
			if (@$_GET["t"] <> "")
				return ($indicador3->TableVar == $_GET["t"]);
		} else {
			return TRUE;
		}
	}
	...
	//  - open connection
	//
	function cindicador3_list() {
		global $conn;

		// Initialize table object
		$GLOBALS["indicador3"] = new cindicador3();
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'indicador3', TRUE);
	...
	//
	function Page_Init() {
		global $gsExport, $gsExportFile, $indicador3;
		global $Security;
	...
	$indicador3->Export = @$_GET["export"]; // Get export parameter
	$gsExport = $indicador3->Export; // Get export parameter, used in header
	$gsExportFile = $indicador3->TableVar; // Get export file, used in header
	if ($indicador3->Export == "print" || $indicador3->Export == "html") {

		// Printer friendly or Export to HTML, no action required
	}
	if ($indicador3->Export == "excel") {
	...
	//
	function Page_Main() {
		global $objForm, $gsSearchError, $Security, $indicador3;
		...
		if ($indicador3->getRecordsPerPage() <> "") {
			$this->lDisplayRecs = $indicador3->getRecordsPerPage(); // Restore from Session
		} else {
		...
		$indicador3->setSessionWhere($sFilter);
		$indicador3->CurrentFilter = "";

		// Export data only
		if (in_array($indicador3->Export, array("html","word","excel","xml","csv"))) {
			$this->ExportData();
	...
	function SetUpDisplayRecs() {
		...
		if ($sWrk <> "") {
			...
			$indicador3->setRecordsPerPage($this->lDisplayRecs); // Save to Session

			// Reset start position
			$this->lStartRec = 1;
			$indicador3->setStartRecordNumber($this->lStartRec);
		}
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			$indicador3->CurrentOrder = ew_StripSlashes(@$_GET["order"]);
			$indicador3->CurrentOrderType = @$_GET["ordertype"];
			$indicador3->UpdateSort($indicador3->idMer); // Field 
			$indicador3->setStartRecordNumber(1); // Reset start position
		}
	}
	...
	function LoadSortOrder() {
		global $indicador3;
		$sOrderBy = $indicador3->getSessionOrderBy(); // Get order by from Session
		if ($sOrderBy == "") {
			if ($indicador3->SqlOrderBy() <> "") {
				$sOrderBy = $indicador3->SqlOrderBy();
				$indicador3->setSessionOrderBy($sOrderBy);
			}
		}
	}
	...
	function SetUpStartRec() {
		...
		$this->lStartRec = $indicador3->getStartRecordNumber();
	...
	function LoadRecordset($offset = -1, $rowcnt = -1) {
		global $conn, $indicador3;

		// Call Recordset Selecting event
		$indicador3->Recordset_Selecting($indicador3->CurrentFilter);

		// Load list page SQL
		$sSql = $indicador3->SelectSQL();
		...
		$indicador3->Recordset_Selected($rs);
		return $rs;
	}

	// Load row based on key values
	function LoadRow() {
		global $conn, $Security, $indicador3;
		$sFilter = $indicador3->KeyFilter();

		// Call Row Selecting event
		$indicador3->Row_Selecting($sFilter);

		// Load sql based on filter
		$indicador3->CurrentFilter = $sFilter;
		$sSql = $indicador3->SQL();
	...
	function LoadRowValues(&$rs) {
		global $indicador3;
		$indicador3->idMer->setDbValue($rs->fields('idMer'));
	}
	...
	function RenderRow() {
		...
		$indicador3->Row_Rendering();

		// Common render codes for all row types
		// idMer

		$indicador3->idMer->CellCssStyle = "";
		$indicador3->idMer->CellCssClass = "";
		if ($indicador3->RowType == EW_ROWTYPE_VIEW) { // View row

			// idMer
			$indicador3->idMer->ViewValue = $indicador3->idMer->CurrentValue;
			$indicador3->idMer->CssStyle = "";
			$indicador3->idMer->CssClass = "";
			$indicador3->idMer->ViewCustomAttributes = "";

			// idMer
			$indicador3->idMer->HrefValue = "";
		}

		// Call Row Rendered event
		$indicador3->Row_Rendered();
	}
	...
	function ExportData() {
		...
		if ($indicador3->Export == "xml") {
			$XmlDoc = new cXMLDocument();
		} else {
			echo ew_ExportHeader($indicador3->Export);

			// Horizontal format, write header
			if ($sExportStyle <> "v" || $indicador3->Export == "csv") {
				$sExportStr = "";
				ew_ExportAddValue($sExportStr, 'idMer', $indicador3->Export);
				echo ew_ExportLine($sExportStr, $indicador3->Export);
			}
		}
		...
		while (!$rs->EOF && $this->lRecCnt < $this->lStopRec) {
			...
			if (intval($this->lRecCnt) >= intval($this->lStartRec)) {
				...
				$indicador3->RowType = EW_ROWTYPE_VIEW; // Render view
				$this->RenderRow();
				if ($indicador3->Export == "xml") {
					$XmlDoc->BeginRow();
					$XmlDoc->AddField('idMer', $indicador3->idMer->CurrentValue);
					$XmlDoc->EndRow();
				} else {
					if ($sExportStyle == "v" && $indicador3->Export <> "csv") { // Vertical format
						echo ew_ExportField('idMer', $indicador3->idMer->ExportValue($indicador3->Export, $indicador3->ExportOriginalValue), $indicador3->Export);
					}	else { // Horizontal format
						$sExportStr = "";
						ew_ExportAddValue($sExportStr, $indicador3->idMer->ExportValue($indicador3->Export, $indicador3->ExportOriginalValue), $indicador3->Export);
						echo ew_ExportLine($sExportStr, $indicador3->Export);
					}
				}
			}
		...
		if ($indicador3->Export == "xml") {
			...
			echo ew_ExportFooter($indicador3->Export);
		}
	}
