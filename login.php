...
login.ValidateForm = function(fobj)
{
	...
	if (!ew_HasValue(fobj.username))
		return ew_OnError(this, fobj.username, "Por favor ingrese una Identificaci�n de Usuario");
	if (!ew_HasValue(fobj.password))
		return ew_OnError(this, fobj.password, "Ingrese Contrasena");
...
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
		<td><span class="phpmaker">Nombre del usuario</span></td>
		<td><span class="phpmaker"><input type="text" name="username" id="username" size="20" value="<?php echo $login->sUsername ?>"></span></td>
	</tr>
	<tr>
            <td><span class="phpmaker">Contrase&ntilde;a</span></td>
		<td><span class="phpmaker"><input type="password" name="password" id="password" size="20"></span></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
		<td><span class="phpmaker">
		<label><input type="radio" name="rememberme" id="rememberme" value="a"<?php if ($login->sLoginType == "a") { ?> checked="checked"<?php } ?>>Ingresar automaticamente hasta que me desconecte explicitamente</label><br>
		<label><input type="radio" name="rememberme" id="rememberme" value="u"<?php if ($login->sLoginType == "u") { ?>  checked="checked"<?php } ?>>Guardar mi nombre de usuario</label><br>
		<label><input type="radio" name="rememberme" id="rememberme" value=""<?php if ($login->sLoginType == "") { ?> checked="checked"<?php } ?>>Siempre preguntar por mi usuario y Contrase&ntilde;a</label>
		</span></td>
	</tr>
	<tr>
...
class clogin {
	...
	function Page_Main() {
		...
		if ($bValidate) {
			...
			if ($bValidate) {
				...
				if (!$bValidPwd)
					$this->setMessage("ID del usuario o Contrase&ntilde;a incorrecta"); // Invalid User ID/password
			} else {
				if ($this->getMessage() == "")
					$this->setMessage("Ingreso cancelado"); // Login cancelled
			}
			if ($bValidPwd) {
				...
				ew_WriteAuditTrailOnLogInOut("login");
	...
	function ValidateForm($usr, $pwd) {
		...
		if (trim($usr) == "") {
			...
			$gsFormError .= "Por favor ingrese una Identificaci�n de Usuario";
		}
		if (trim($pwd) == "") {
			$gsFormError .= ($gsFormError <> "") ? "<br>" : "";
