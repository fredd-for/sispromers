...
$idMetaReporte=$_REQUEST['idMetaReporte'];
$idConsultoria=$_REQUEST['idConsultoria'];
$idMeta=$_REQUEST['idMeta'];

mysql_select_db($database_conexion, $conexion);
$query_MRC= "SELECT concat(c.nombre,' ',c.paterno,' ',c.materno) as nombre, a.idMetaReporte, a.observacion, a.archivo, a.fechaCreacion,
(case a.estado when 1 then 'Corregir' when 2 then 'Revision' when 3 then 'Aprobado' end) as estado
FROM meta_reporte_control a, responsable_consultoria b, usuario c
WHERE a.idMetaReporte=$idMetaReporte AND a.idRC =b.idRC AND b.idUsuario=c.idUsuario";
$mostrar_MRC=mysql_query($query_MRC, $conexion) or die(mysql_error());
$total_MRC=mysql_num_rows($mostrar_MRC);

mysql_select_db($database_conexion, $conexion);
$query_RC= "SELECT idRC FROM responsable_consultoria WHERE idUsuario=".$_SESSION['idUsuario']." AND idConsultoria=$idConsultoria";
$mostrar_RC=mysql_query($query_RC, $conexion) or die(mysql_error());
$row_RC=mysql_fetch_assoc($mostrar_RC);
$idRC=$row_RC['idRC'];

if($idRC>0){
mysql_select_db($database_conexion, $conexion);
$query_MRCestado= "SELECT idMetaReporteControl FROM meta_reporte_control WHERE idMetaReporte=$idMetaReporte AND idRC=$idRC";
$mostrar_MRCestado=mysql_query($query_MRCestado, $conexion) or die(mysql_error());
$total_MRCestado=mysql_num_rows($mostrar_MRCestado);
}else{$total_MRCestado=1;}

mysql_select_db($database_conexion, $conexion);
$query_metaestado= "SELECT b.estado FROM meta_reporte a, meta b WHERE a.idMetaReporte=$idMetaReporte AND a.idMeta=b.idMeta";
$mostrar_metaestado=mysql_query($query_metaestado, $conexion) or die(mysql_error());
$row_metaestado=mysql_fetch_assoc($mostrar_metaestado);
?>
...
<tr class="ewTableHeader">
    ...
    <td>Revision Informe</td>
    <td>Observaci&oacute;n</td>
    <td>Estado</td>
    <td>Fecha Revision</td>
    <td  align="right"><img src="images/close.png" id="cerrar_revisar_hito" style="cursor: pointer" title="Cerrar" onclick="func_cerrar_revisar_meta(<?php echo $idMeta?>)"></td>
</tr>
<?php
while($row_MRC=mysql_fetch_assoc($mostrar_MRC)){?>
<tr>
    <td><?php echo utf8_encode($row_MRC['nombre'])?></td>
    <td><?php echo "<a href=\"files_consultoria/".utf8_encode($row_MRC['archivo'])."\" title='Extraer archivo'>".utf8_encode($row_MRC['archivo'])."</a>";?></td>
    <td><?php echo utf8_encode($row_MRC['observacion'])?></td>
    <td><?php echo $row_MRC['estado']?></td>
    <td><?php echo date("d-m-Y",strtotime($row_MRC['fechaCreacion']))?></td>
    <td>
    </td>
</tr>
<?php }
if($total_MRCestado<1 && $row_metaestado['estado']!=3){
?>
<tr>
    <td><?php echo $_SESSION['nombre']." ".$_SESSION['paterno'];?></td>
    <td><input type="file" name="archivo_revision_meta<?php echo $idMetaReporte?>" id="archivo_revision_meta<?php echo $idMetaReporte?>" size="15"></td>
    <td><textarea name="observacion_revision_meta<?php echo $idMetaReporte?>" id="observacion_revision_meta<?php echo $idMetaReporte?>" rows="3" cols="30"></textarea></td>
    <td></td>
    <td></td>
    <td>
        <input type="hidden" value="<?php echo $idConsultoria?>">
        <input type="button" name="aprobar_reporte_meta" value="Aprobar" style="cursor: pointer" onclick="func_aprobar_reporte_meta(<?php echo $idMetaReporte?>,<?php echo $idRC?>,<?php echo $idMeta?>,<?php echo $idConsultoria?>)">
        <input type="button" name="observar_reporte_meta" value="Observar" style="cursor: pointer" onclick="func_observar_reporte_meta(<?php echo $idMetaReporte?>,<?php echo $idRC?>,<?php echo $idMeta?>)">
    </td>
</tr>
