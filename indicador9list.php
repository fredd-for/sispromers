...
<?php include "indicador9info.php" ?>
...
// Define page object
$indicador9_list = new cindicador9_list();
$Page =& $indicador9_list;

// Page init processing
$indicador9_list->Page_Init();

// Page main processing
$indicador9_list->Page_Main();
?>
...
if(isset($_POST['x_formalizada'])){if($_POST['x_formalizada']=='1')
    {$query_mer.=" AND a.idMer=g.idMer AND g.idPlanilla='12' AND g.porcentaje>'0' ";}}
else{$query_mer.=" AND a.idMer=g.idMer AND g.idPlanilla='12' AND g.porcentaje>'0' ";}
...
?>
<form method="post" action="indicador9list.php">
...
<table>
     <tr style="background: #667120; color:#ffffff; font-size: 15px">
         <th colspan="2">INDICADOR C.2-I.1:</th><td colspan="16">Porcentaje de MERS que han prestado servicios a entidades p&uacute;blicas o privadas, con contrato formal: A&ntilde;o 2: 30%; Fin de proyecto: 80% de las 115 MERS.</td>
     </tr>
</table>
...
mysql_select_db($database_conexion, $conexion);
$query_formulario12 = "SELECT tipoContrato, concat(descripcion,' ',cantidad,' ',unidad,' = ',costoTotal, ' Bs.') as detalle, costoTotal,cumple FROM formulario12 WHERE idMer='".$row_mer['idMer']."' AND idPlanilla=12";
...
<?php } ?>
    ...
    <table>
     ...
     <tr style="background:#b8f5e0; font-size: 11px; text-align: center">
         <th colspan="6">N&uacute;mero Mers que prestaron servicios: <?php echo $totalRows_mer; ?></th><td colspan="12">Porcentaje de Cumplimiento : <?php printf("%0.2f", ($totalMers*100)/115); echo" %"; ?></td>
     </tr>
...
// Page Class
//
class cindicador9_list {

	// Page ID
	var $PageID = 'list';

	// Table Name
	var $TableName = 'indicador9';

	// Page Object Name
	var $PageObjName = 'indicador9_list';
	...
	function PageUrl() {
		...
		global $indicador9;
		if ($indicador9->UseTokenInUrl) $PageUrl .= "t=" . $indicador9->TableVar . "&"; // add page token
		return $PageUrl;
	}
	...
	function IsPageRequest() {
		global $objForm, $indicador9;
		if ($indicador9->UseTokenInUrl) {

			//IsPageRequest = False
			if ($objForm)
				return ($indicador9->TableVar == $objForm->GetValue("t"));
			if (@$_GET["t"] <> "")
				return ($indicador9->TableVar == $_GET["t"]);
		} else {
			return TRUE;
		}
	}
	...
	//  - open connection
	//
	function cindicador9_list() {
		global $conn;

		// Initialize table object
		$GLOBALS["indicador9"] = new cindicador9();
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'indicador9', TRUE);
	...
	//
	function Page_Init() {
		global $gsExport, $gsExportFile, $indicador9;
		global $Security;
	...
	$indicador9->Export = @$_GET["export"]; // Get export parameter
	$gsExport = $indicador9->Export; // Get export parameter, used in header
	$gsExportFile = $indicador9->TableVar; // Get export file, used in header
	if ($indicador9->Export == "print" || $indicador9->Export == "html") {

		// Printer friendly or Export to HTML, no action required
	}
	if ($indicador9->Export == "excel") {
	...
	//
	function Page_Main() {
		global $objForm, $gsSearchError, $Security, $indicador9;
		...
		if ($indicador9->getRecordsPerPage() <> "") {
			$this->lDisplayRecs = $indicador9->getRecordsPerPage(); // Restore from Session
		} else {
		...
		$indicador9->setSessionWhere($sFilter);
		$indicador9->CurrentFilter = "";

		// Export data only
		if (in_array($indicador9->Export, array("html","word","excel","xml","csv"))) {
			$this->ExportData();
	...
	function SetUpDisplayRecs() {
		...
		if ($sWrk <> "") {
			...
			$indicador9->setRecordsPerPage($this->lDisplayRecs); // Save to Session

			// Reset start position
			$this->lStartRec = 1;
			$indicador9->setStartRecordNumber($this->lStartRec);
		}
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			$indicador9->CurrentOrder = ew_StripSlashes(@$_GET["order"]);
			$indicador9->CurrentOrderType = @$_GET["ordertype"];
			$indicador9->UpdateSort($indicador9->idMer); // Field 
			$indicador9->setStartRecordNumber(1); // Reset start position
		}
	}
	...
	function LoadSortOrder() {
		global $indicador9;
		$sOrderBy = $indicador9->getSessionOrderBy(); // Get order by from Session
		if ($sOrderBy == "") {
			if ($indicador9->SqlOrderBy() <> "") {
				$sOrderBy = $indicador9->SqlOrderBy();
				$indicador9->setSessionOrderBy($sOrderBy);
			}
		}
	}
	...
	function SetUpStartRec() {
		...
		$this->lStartRec = $indicador9->getStartRecordNumber();
	...
	function LoadRecordset($offset = -1, $rowcnt = -1) {
		global $conn, $indicador9;

		// Call Recordset Selecting event
		$indicador9->Recordset_Selecting($indicador9->CurrentFilter);

		// Load list page SQL
		$sSql = $indicador9->SelectSQL();
		...
		$indicador9->Recordset_Selected($rs);
		return $rs;
	}

	// Load row based on key values
	function LoadRow() {
		global $conn, $Security, $indicador9;
		$sFilter = $indicador9->KeyFilter();

		// Call Row Selecting event
		$indicador9->Row_Selecting($sFilter);

		// Load sql based on filter
		$indicador9->CurrentFilter = $sFilter;
		$sSql = $indicador9->SQL();
	...
	function LoadRowValues(&$rs) {
		global $indicador9;
		$indicador9->idMer->setDbValue($rs->fields('idMer'));
	}
	...
	function RenderRow() {
		...
		$indicador9->Row_Rendering();

		// Common render codes for all row types
		// idMer

		$indicador9->idMer->CellCssStyle = "";
		$indicador9->idMer->CellCssClass = "";
		if ($indicador9->RowType == EW_ROWTYPE_VIEW) { // View row

			// idMer
			$indicador9->idMer->ViewValue = $indicador9->idMer->CurrentValue;
			$indicador9->idMer->CssStyle = "";
			$indicador9->idMer->CssClass = "";
			$indicador9->idMer->ViewCustomAttributes = "";

			// idMer
			$indicador9->idMer->HrefValue = "";
		}

		// Call Row Rendered event
		$indicador9->Row_Rendered();
	}
	...
	function ExportData() {
		...
		if ($indicador9->Export == "xml") {
			$XmlDoc = new cXMLDocument();
		} else {
			echo ew_ExportHeader($indicador9->Export);

			// Horizontal format, write header
			if ($sExportStyle <> "v" || $indicador9->Export == "csv") {
				$sExportStr = "";
				ew_ExportAddValue($sExportStr, 'idMer', $indicador9->Export);
				echo ew_ExportLine($sExportStr, $indicador9->Export);
			}
		}
		...
		while (!$rs->EOF && $this->lRecCnt < $this->lStopRec) {
			...
			if (intval($this->lRecCnt) >= intval($this->lStartRec)) {
				...
				$indicador9->RowType = EW_ROWTYPE_VIEW; // Render view
				$this->RenderRow();
				if ($indicador9->Export == "xml") {
					$XmlDoc->BeginRow();
					$XmlDoc->AddField('idMer', $indicador9->idMer->CurrentValue);
					$XmlDoc->EndRow();
				} else {
					if ($sExportStyle == "v" && $indicador9->Export <> "csv") { // Vertical format
						echo ew_ExportField('idMer', $indicador9->idMer->ExportValue($indicador9->Export, $indicador9->ExportOriginalValue), $indicador9->Export);
					}	else { // Horizontal format
						$sExportStr = "";
						ew_ExportAddValue($sExportStr, $indicador9->idMer->ExportValue($indicador9->Export, $indicador9->ExportOriginalValue), $indicador9->Export);
						echo ew_ExportLine($sExportStr, $indicador9->Export);
					}
				}
			}
		...
		if ($indicador9->Export == "xml") {
			...
			echo ew_ExportFooter($indicador9->Export);
		}
	}
