...
<?php include "indicador8info.php" ?>
...
// Define page object
$indicador8_list = new cindicador8_list();
$Page =& $indicador8_list;

// Page init processing
$indicador8_list->Page_Init();

// Page main processing
$indicador8_list->Page_Main();
?>
...
?>
<form method="post" action="indicador8list.php">
    ...
    <tr style="color: blue">
     ...
     <td>
         <select name="x_formalizada" id="x_formalizada">
             ...
             <option value="1" <?php if(isset ($_POST['x_formalizada'])){if($_POST['x_formalizada']=='1'){echo "selected";$sw='1';}}else{echo "selected";$sw='1';}?>>Mers con Mora</option>
             <option value="0" <?php if($_POST['x_formalizada']=='0'){echo "selected";$sw='0';}?>>Mers sin Mora</option>
         </select>
     </td>
     </tr>
   <tr style="color: blue">
...
<table>
     <tr style="background: #667120; color:#ffffff; font-size: 15px">
         <th colspan="2">INDICADOR C.1-I.4:</th><td colspan="10">Porcentaje m&aacute;ximo de MERS con problemas de mora en el financiamiento durante todo el proyecto: 10% de las 115  MERS</td>
     </tr>
</table>
<table id="table_example" width="100%" border="1" class="ewTable">
    ...
    <tr>
        ...
        <th colspan="3" align="center" bgcolor="#b8f5e0">DOCUMENTO</th>
...
if($row_formulario16['porcentaje']=='1'){$indicador2='1';}else{$indicador2='0';}
...
<td><?php echo $contador;?></td>
       ...
       <tr <?php if($row_formulario16['cuenta']=='SI'){echo "style='color:blue'"; $tiqueado="&radic;";}else{echo "style='color:gray'";$tiqueado="";}?>>
        ...
        <td <?php if($row_formulario16['porcentaje']=='1'){echo "style='color:blue'"; $tiqueado="&radic;";}else{echo "style='color:gray'";$tiqueado="";}?>>Mora <?php echo $tiqueado ?></td>
        <td <?php if($row_obtencionCredito['prestamo']>'0'){echo "style='color:blue'"; $prestamo=$row_obtencionCredito['prestamo'];}else{echo "style='color:gray'";$prestamo="";}?>><?php echo $prestamo;?></td>
<td <?php if($indicador2=='1'){ echo "style='background:#C2D69A'"; $resp='Mora';$contIndicador=$contIndicador+1;}else{  echo "style='background:#FFFF99'"; $resp='Sin Mora';}?>><?php echo $resp;?></td>
    </tr>
<?php
$contador++;
}
...
<table>
     ...
     <tr style="background:#b8f5e0; font-size: 11px; text-align: center">
         <th colspan="6">N&uacute;mero de MERS con mora: <?php echo $contIndicador; ?></th><td colspan="6">Porcentaje de mora : <?php printf("%0.2f", ($contIndicador*100)/115); echo" %"; ?></td>
     </tr>
...
// Page Class
//
class cindicador8_list {

	// Page ID
	var $PageID = 'list';

	// Table Name
	var $TableName = 'indicador8';

	// Page Object Name
	var $PageObjName = 'indicador8_list';
	...
	function PageUrl() {
		...
		global $indicador8;
		if ($indicador8->UseTokenInUrl) $PageUrl .= "t=" . $indicador8->TableVar . "&"; // add page token
		return $PageUrl;
	}
	...
	function IsPageRequest() {
		global $objForm, $indicador8;
		if ($indicador8->UseTokenInUrl) {

			//IsPageRequest = False
			if ($objForm)
				return ($indicador8->TableVar == $objForm->GetValue("t"));
			if (@$_GET["t"] <> "")
				return ($indicador8->TableVar == $_GET["t"]);
		} else {
			return TRUE;
		}
	}
	...
	//  - open connection
	//
	function cindicador8_list() {
		global $conn;

		// Initialize table object
		$GLOBALS["indicador8"] = new cindicador8();
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'indicador8', TRUE);
	...
	//
	function Page_Init() {
		global $gsExport, $gsExportFile, $indicador8;
		global $Security;
	...
	$indicador8->Export = @$_GET["export"]; // Get export parameter
	$gsExport = $indicador8->Export; // Get export parameter, used in header
	$gsExportFile = $indicador8->TableVar; // Get export file, used in header
	if ($indicador8->Export == "print" || $indicador8->Export == "html") {

		// Printer friendly or Export to HTML, no action required
	}
	if ($indicador8->Export == "excel") {
	...
	//
	function Page_Main() {
		global $objForm, $gsSearchError, $Security, $indicador8;
		...
		if ($indicador8->getRecordsPerPage() <> "") {
			$this->lDisplayRecs = $indicador8->getRecordsPerPage(); // Restore from Session
		} else {
		...
		$indicador8->setSessionWhere($sFilter);
		$indicador8->CurrentFilter = "";

		// Export data only
		if (in_array($indicador8->Export, array("html","word","excel","xml","csv"))) {
			$this->ExportData();
	...
	function SetUpDisplayRecs() {
		...
		if ($sWrk <> "") {
			...
			$indicador8->setRecordsPerPage($this->lDisplayRecs); // Save to Session

			// Reset start position
			$this->lStartRec = 1;
			$indicador8->setStartRecordNumber($this->lStartRec);
		}
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			$indicador8->CurrentOrder = ew_StripSlashes(@$_GET["order"]);
			$indicador8->CurrentOrderType = @$_GET["ordertype"];
			$indicador8->UpdateSort($indicador8->idMer); // Field 
			$indicador8->setStartRecordNumber(1); // Reset start position
		}
	}
	...
	function LoadSortOrder() {
		global $indicador8;
		$sOrderBy = $indicador8->getSessionOrderBy(); // Get order by from Session
		if ($sOrderBy == "") {
			if ($indicador8->SqlOrderBy() <> "") {
				$sOrderBy = $indicador8->SqlOrderBy();
				$indicador8->setSessionOrderBy($sOrderBy);
			}
		}
	}
	...
	function SetUpStartRec() {
		...
		$this->lStartRec = $indicador8->getStartRecordNumber();
	...
	function LoadRecordset($offset = -1, $rowcnt = -1) {
		global $conn, $indicador8;

		// Call Recordset Selecting event
		$indicador8->Recordset_Selecting($indicador8->CurrentFilter);

		// Load list page SQL
		$sSql = $indicador8->SelectSQL();
		...
		$indicador8->Recordset_Selected($rs);
		return $rs;
	}

	// Load row based on key values
	function LoadRow() {
		global $conn, $Security, $indicador8;
		$sFilter = $indicador8->KeyFilter();

		// Call Row Selecting event
		$indicador8->Row_Selecting($sFilter);

		// Load sql based on filter
		$indicador8->CurrentFilter = $sFilter;
		$sSql = $indicador8->SQL();
	...
	function LoadRowValues(&$rs) {
		global $indicador8;
		$indicador8->idMer->setDbValue($rs->fields('idMer'));
	}
	...
	function RenderRow() {
		...
		$indicador8->Row_Rendering();

		// Common render codes for all row types
		// idMer

		$indicador8->idMer->CellCssStyle = "";
		$indicador8->idMer->CellCssClass = "";
		if ($indicador8->RowType == EW_ROWTYPE_VIEW) { // View row

			// idMer
			$indicador8->idMer->ViewValue = $indicador8->idMer->CurrentValue;
			$indicador8->idMer->CssStyle = "";
			$indicador8->idMer->CssClass = "";
			$indicador8->idMer->ViewCustomAttributes = "";

			// idMer
			$indicador8->idMer->HrefValue = "";
		}

		// Call Row Rendered event
		$indicador8->Row_Rendered();
	}
	...
	function ExportData() {
		...
		if ($indicador8->Export == "xml") {
			$XmlDoc = new cXMLDocument();
		} else {
			echo ew_ExportHeader($indicador8->Export);

			// Horizontal format, write header
			if ($sExportStyle <> "v" || $indicador8->Export == "csv") {
				$sExportStr = "";
				ew_ExportAddValue($sExportStr, 'idMer', $indicador8->Export);
				echo ew_ExportLine($sExportStr, $indicador8->Export);
			}
		}
		...
		while (!$rs->EOF && $this->lRecCnt < $this->lStopRec) {
			...
			if (intval($this->lRecCnt) >= intval($this->lStartRec)) {
				...
				$indicador8->RowType = EW_ROWTYPE_VIEW; // Render view
				$this->RenderRow();
				if ($indicador8->Export == "xml") {
					$XmlDoc->BeginRow();
					$XmlDoc->AddField('idMer', $indicador8->idMer->CurrentValue);
					$XmlDoc->EndRow();
				} else {
					if ($sExportStyle == "v" && $indicador8->Export <> "csv") { // Vertical format
						echo ew_ExportField('idMer', $indicador8->idMer->ExportValue($indicador8->Export, $indicador8->ExportOriginalValue), $indicador8->Export);
					}	else { // Horizontal format
						$sExportStr = "";
						ew_ExportAddValue($sExportStr, $indicador8->idMer->ExportValue($indicador8->Export, $indicador8->ExportOriginalValue), $indicador8->Export);
						echo ew_ExportLine($sExportStr, $indicador8->Export);
					}
				}
			}
		...
		if ($indicador8->Export == "xml") {
			...
			echo ew_ExportFooter($indicador8->Export);
		}
	}
