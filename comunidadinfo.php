...
// PHPMaker 6 configuration for Table comunidad
$comunidad = NULL; // Initialize table object
...
class ccomunidad {
	...
	function ccomunidad() {
		$this->TableVar = "comunidad";
		$this->TableName = "comunidad";
		$this->SelectLimit = TRUE;
		$this->idComunidad = new cField('comunidad', 'x_idComunidad', 'idComunidad', "`idComunidad`", 3, -1, FALSE);
		$this->fields['idComunidad'] =& $this->idComunidad;
		$this->idRegional = new cField('comunidad', 'x_idRegional', 'idRegional', "`idRegional`", 3, -1, FALSE);
		$this->fields['idRegional'] =& $this->idRegional;
		$this->idDepartamento = new cField('comunidad', 'x_idDepartamento', 'idDepartamento', "`idDepartamento`", 3, -1, FALSE);
		$this->fields['idDepartamento'] =& $this->idDepartamento;
		$this->idMunicipio = new cField('comunidad', 'x_idMunicipio', 'idMunicipio', "`idMunicipio`", 3, -1, FALSE);
		$this->fields['idMunicipio'] =& $this->idMunicipio;
		$this->comunidad = new cField('comunidad', 'x_comunidad', 'comunidad', "`comunidad`", 200, -1, FALSE);
		$this->fields['comunidad'] =& $this->comunidad;
	}
	...
	function InsertSQL(&$rs) {
		...
		return "INSERT INTO `comunidad` ($names) VALUES ($values)";
	}
	...
	function DeleteSQL(&$rs) {
		$SQL = "DELETE FROM `comunidad` WHERE ";
		$SQL .= EW_DB_QUOTE_START . 'idComunidad' . EW_DB_QUOTE_END . '=' .	ew_QuotedValue($rs['idComunidad'], $this->idComunidad->FldDataType) . ' AND ';
	...
	function KeyFilter() {
		...
		if (!is_numeric($this->idComunidad->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idComunidad@", ew_AdjustSql($this->idComunidad->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View url
	function ViewUrl() {
		return $this->KeyUrl("comunidadview.php", $this->UrlParm());
	}

	// Add url
	function AddUrl() {
		$AddUrl = "comunidadadd.php";
	...
	// Edit url
	function EditUrl() {
		return $this->KeyUrl("comunidadedit.php", $this->UrlParm());
	}
	...
	// Copy url
	function CopyUrl() {
		return $this->KeyUrl("comunidadadd.php", $this->UrlParm());
	}
	...
	// Delete url
	function DeleteUrl() {
		return $this->KeyUrl("comunidaddelete.php", $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idComunidad->CurrentValue)) {
			$sUrl .= "idComunidad=" . urlencode($this->idComunidad->CurrentValue);
		} else {
	...
	function UrlParm($parm = "") {
		$UrlParm = ($this->UseTokenInUrl) ? "t=comunidad" : "";
		if ($parm <> "") {
			if ($UrlParm <> "")
				$UrlParm .= "&";
			$UrlParm .= $parm;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->comunidad->setDbValue($rs->fields('comunidad'));
	}
	...
	function RenderListRow() {
		...
		// comunidad
		$this->comunidad->ViewValue = $this->comunidad->CurrentValue;
		$this->comunidad->CssStyle = "";
		$this->comunidad->CssClass = "";
		$this->comunidad->ViewCustomAttributes = "";
