...
$idCronogramaReporte=$_REQUEST['idCronogramaReporte'];

mysql_select_db($database_conexion, $conexion);
$query_CRC= "SELECT c.nombre,c.paterno,c.materno, a.idCronogramaReporte, a.observacion, a.archivo, a.fechaCreacion,
(case a.estado when 1 then 'Corregir' when 2 then 'Revision' when 3 then 'Aprobado' end) as estado
FROM cronograma_reporte_control a, responsable_consultoria b, usuario c
WHERE idCronogramaReporte=$idCronogramaReporte AND a.idRC =b.idRC AND b.idUsuario=c.idUsuario";
...
if($idRC>0){
mysql_select_db($database_conexion, $conexion);
$query_CRCestado= "SELECT idCronogramaReporteControl FROM cronograma_reporte_control WHERE idCronogramaReporte=$idCronogramaReporte AND idRC=$idRC";
...
<tr class="ewTableHeader">
    ...
    <td  align="right"><img src="images/close.png" id="cerrar_revisar_hito" style="cursor: pointer" title="Cerrar" onclick="func_cerrar_revisar_hito()"></td>
</tr>
<?php
while($row_CRC=mysql_fetch_assoc($mostrar_CRC)){?>
<tr>
    <td><?php echo utf8_encode($row_CRC['nombre']." ".$row_CRC['paterno']." ".$row_CRC['materno'])?></td>
    <td><?php echo "<a href=\"files_consultoria/".utf8_encode($row_CRC['archivo'])."\" title='Extraer archivo'>".utf8_encode($row_CRC['archivo'])."</a>";?></td>
    <td><?php echo utf8_encode($row_CRC['observacion'])?></td>
    <td><?php echo $row_CRC['estado']?></td>
    <td><?php echo date("d-m-Y",strtotime($row_CRC['fechaCreacion']))?></td>
    <td>
    </td>
</tr>
<?php }
if($total_CRCestado<1){
?>
<form method="post" action="avance_producto_guardar.php" enctype="multipart/form-data" >
<tr >
    <td><?php echo $_SESSION['nombre']." ".$_SESSION['paterno'];?></td>
    <td><input type="file" name="archivo_revision_hito" id="archivo_revision_hito" size="35"></td>
    <td><textarea name="observacion_revision_hito" id="observacion_revision_hito" rows="2" cols="50"></textarea></td>
    <td></td>
    <td></td>
    <td>
        <input type="hidden" name="x_idRC" value="<?php echo $idRC?>">
        <input type="hidden" name="x_idCronogramaReporte" value="<?php echo $idCronogramaReporte?>">
        ...
        <input type="submit" name="aprobar_cronograma_reporte" value="Aprobar" style="cursor: pointer" onclick="return validar_formulario_revision_hito_aprobar(this);">
        <input type="submit" name="observar_cronograma_reporte" value="Observar" style="cursor: pointer" onclick="return validar_formulario_revision_hito_observar(this);">
    </td>
</tr>
</form>
<?php }?>
