...
$query = "SELECT a.detalle FROM cronograma AS a WHERE a.idCronograma =  '$v_idCronograma' AND a.idConsultoria =  '$v_idConsultoria'";
$mostrar_hito = mysql_query($query, $conexion) or die(mysql_error());
$row_hito = mysql_fetch_assoc($mostrar_hito);

$zipfile = new zipfile();

// Download Archivos Reporte Hito Consultor
mysql_select_db($database_conexion, $conexion);
$query = "SELECT cronograma_reporte.archivo FROM cronograma_reporte WHERE cronograma_reporte.idCronograma =  '$v_idCronograma'  AND cronograma_reporte.archivo!=''";
$mostrar_reporte = mysql_query($query, $conexion) or die(mysql_error());
$row_num_reporte = mysql_num_rows($mostrar_reporte);
if ($row_num_reporte > 0) {
    $zipfile->add_dir("ReporteHitoConsultor/");
    while ($row_reporte = mysql_fetch_assoc($mostrar_reporte)) {
        $dir_file = "files_consultoria/" . $row_reporte['archivo'];
        $zipfile->add_file(implode("", file($dir_file)), "ReporteHitoConsultor/" . $row_reporte['archivo']);
        //$zipfile->add_file(file($dir_file), "ReporteHitoConsultor/" . utf8_encode($row_reporte['archivo']));
    }
}

// Download Archivos Revision hito
mysql_select_db($database_conexion, $conexion);
$query = "SELECT b.archivo FROM cronograma_reporte AS a
Inner Join cronograma_reporte_control AS b ON a.idCronogramaReporte = b.idCronogramaReporte
WHERE a.idCronograma = '$v_idCronograma' AND b.archivo!=''";
$mostrar_repControl = mysql_query($query, $conexion) or die(mysql_error());
$row_num_repControl = mysql_num_rows($mostrar_repControl);
if ($row_num_repControl > 0) {
    $zipfile->add_dir("RevisionHito/");
    while ($row_reporte = mysql_fetch_assoc($mostrar_repControl)) {
        $dir_file = "files_consultoria/" . $row_reporte['archivo'];
        $zipfile->add_file(implode("", file($dir_file)), "RevisionHito/" . $row_reporte['archivo']);
        //$zipfile->add_file(file($dir_file), "ReporteHitoConsultor/" . utf8_encode($row_reporte['archivo']));
    }
}

// Download Archivos Meta
mysql_select_db($database_conexion, $conexion);
$query = "SELECT a.idMeta,a.meta FROM meta AS a WHERE a.cabecera =  '0' AND a.idCronograma =  '$v_idCronograma' ";
$mostrar_meta = mysql_query($query, $conexion) or die(mysql_error());
$row_num_meta = mysql_num_rows($mostrar_meta);
while ($row_meta = mysql_fetch_assoc($mostrar_meta)) {
    mysql_select_db($database_conexion, $conexion);
    $query = " SELECT a.archivo FROM meta_reporte_unitario AS a WHERE a.idMeta =  '" . $row_meta['idMeta'] . "' AND a.archivo <>  '' ";
    $mostrar_mru = mysql_query($query, $conexion) or die(mysql_error());
    $row_num_mru = mysql_num_rows($mostrar_mru);

    mysql_select_db($database_conexion, $conexion);
    $query = " SELECT a.archivo FROM meta_reporte AS a WHERE a.idMeta = '" . $row_meta['idMeta'] . "' AND a.archivo <> '' ";
    $mostrar_mr = mysql_query($query, $conexion) or die(mysql_error());
    $row_num_mr = mysql_num_rows($mostrar_mr);

    mysql_select_db($database_conexion, $conexion);
    $query = "SELECT b.archivo FROM meta_reporte AS a Inner Join meta_reporte_control AS b ON a.idMetaReporte = b.idMetaReporte
    WHERE a.idMeta = '" . $row_meta['idMeta'] . "' AND b.archivo <> '' ";
    $mostrar_mrc = mysql_query($query, $conexion) or die(mysql_error());
    $row_num_mrc = mysql_num_rows($mostrar_mrc);

    if ($row_num_mru > 0 || $row_num_mr > 0 || $row_num_mrc > 0) {
        //$zipfile->add_dir("idMeta".$row_meta['idMeta']."/");
        $zipfile->add_dir("idMeta:".$row_meta['idMeta']."/");
        if ($row_num_mru > 0) {
            while ($row = mysql_fetch_assoc($mostrar_mru)) {
                $dir_file = "files_consultoria/" . $row['archivo'];
                $zipfile->add_file(implode("", file($dir_file)), "idMeta:".$row_meta['idMeta']."/" . $row['archivo']);
                //$zipfile->add_file(file($dir_file), "ReporteHitoConsultor/" . utf8_encode($row_reporte['archivo']));
            }
        }
        if ($row_num_mr > 0) {
            while ($row = mysql_fetch_assoc($mostrar_mr)) {
                $dir_file = "files_consultoria/" . $row['archivo'];
                $zipfile->add_file(implode("", file($dir_file)), "idMeta:".$row_meta['idMeta']."/" . $row['archivo']);
                //$zipfile->add_file(file($dir_file), "ReporteHitoConsultor/" . utf8_encode($row_reporte['archivo']));
            }
        ...
        if ($row_num_mrc > 0) {
            while ($row = mysql_fetch_assoc($mostrar_mrc)) {
                $dir_file = "files_consultoria/" . $row['archivo'];
                $zipfile->add_file(implode("", file($dir_file)), "idMeta:".$row_meta['idMeta']."/" . $row['archivo']);
                //$zipfile->add_file(file($dir_file), "ReporteHitoConsultor/" . utf8_encode($row_reporte['archivo']));
            }
...
header("Content-disposition: attachment; filename=HITO" . $row_hito['detalle'] . ".zip");
echo $zipfile->file();
?>
