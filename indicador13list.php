...
if(isset($_POST['x_formalizada'])){if($_POST['x_formalizada']=='1')
    {$query_mer.=" AND a.idMer IN (SELECT idMer FROM formulario WHERE idPlanilla = '13' AND cuenta =  'SI') ";}}
else{$query_mer.=" AND a.idMer IN (SELECT idMer FROM formulario WHERE idPlanilla = '13' AND cuenta =  'SI') ";}

if($_POST['x_formalizada']=='2'){$query_mer.=" AND a.idMer NOT IN (SELECT idMer FROM formulario WHERE idPlanilla =  '13' AND cuenta =  'SI') ";}
...
?>
<form method="post" action="indicador13list.php">
...
<table cellspacing="0" rowhighlightclass="ewTableHighlightRow" rowselectclass="ewTableSelectRow" roweditclass="ewTableEditRow" class="ewTable ewTableSeparate">
    <tr style="color: blue">
     ...
     <td>Mers Part. Evento Municipio</td>
     <td>
         <select name="x_formalizada" id="x_formalizada">
             ...
             <option value="1" <?php if(isset ($_POST['x_formalizada'])){if($_POST['x_formalizada']=='1'){echo "selected";$sw='1';}}else{echo "selected";$sw='1';}?>>Mers con Eventos Municipio</option>
...
<table>
     <tr style="background: #667120; color:#ffffff; font-size: 15px">
         <th colspan="2">INDICADOR C.3-I.2:</th><td colspan="16">Municipios que incorporan acciones de apoyo al sector productivo en sus Planes Anuales y los ejecutan. A&ntilde;o 2: 4; Fin de proyecto: 8</td>
     </tr>
</table>
 <table id="table_example" width="100%" border="1" class="ewTable">
    ...
    <tr class="ewTableHeader">
        ...
        <th style="cursor: pointer" title="Ocultar/Ver Comentarios" onclick="verOcultarFormalizacion();">Mers Part. Eventos Municipio <img src="images/expand.gif"></th>
        <th>Valor</th>
    </tr>
</thead>
...
mysql_select_db($database_conexion, $conexion);
$query_formulario = "SELECT * FROM formulario WHERE idMer='".$row_mer['idMer']."' AND idPlanilla=13";
...
$total_formulario=mysql_num_rows($mostrar_formulario);
...
<td><?php echo $contador;?></td>
        ...
        <td><?php if($row_formulario['cuenta']=='SI'){?><table>
                <tr>
                    <td><?php echo "Aporte del Municipio=".$row_formulario['porcentaje']." Bs.";?></td>
                    <td id="detalle_<?php echo $contadorInd; $contadorInd++;?>"><?php echo $row_formulario['observacion'];?></td>
                </tr>
            </table>
            <?php } ?>
        </td>
<td <?php if($row_formulario['cuenta']=='SI'){
...
<?php
mysql_select_db($database_conexion, $conexion);
$query_numMunicipios= "SELECT idMunicipio FROM mer WHERE idMer IN(SELECT idMer FROM formulario WHERE idPlanilla=13 AND cuenta='SI') GROUP BY idMunicipio";
...
?>
    <table>
     ...
     <tr style="background:#b8f5e0; font-size: 11px; text-align: center">
         <th colspan="6">N&uacute;mero Municipios que incomporan acciones de apoyo en sus planes: <?php echo $total_numMunicipios; ?></th><td colspan="12">Porcentaje de Cumplimiento : <?php printf("%0.2f", ($total_numMunicipios*100)/8); echo" %"; ?></td>
     </tr>
...
class cindicador13_list {
	...
	function PageUrl() {
		...
		global $indicador13;
		if ($indicador13->UseTokenInUrl) $PageUrl .= "t=" . $indicador13->TableVar . "&"; // add page token
		return $PageUrl;
	}
