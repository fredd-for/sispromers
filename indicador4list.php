...
<?php include "indicador4info.php" ?>
...
// Define page object
$indicador4_list = new cindicador4_list();
$Page =& $indicador4_list;

// Page init processing
$indicador4_list->Page_Init();

// Page main processing
$indicador4_list->Page_Main();
?>
...
?>
<form method="post" action="indicador4list.php">
...
// Page Class
//
class cindicador4_list {

	// Page ID
	var $PageID = 'list';

	// Table Name
	var $TableName = 'indicador4';

	// Page Object Name
	var $PageObjName = 'indicador4_list';
	...
	function PageUrl() {
		...
		global $indicador4;
		if ($indicador4->UseTokenInUrl) $PageUrl .= "t=" . $indicador4->TableVar . "&"; // add page token
		return $PageUrl;
	}
	...
	function IsPageRequest() {
		global $objForm, $indicador4;
		if ($indicador4->UseTokenInUrl) {

			//IsPageRequest = False
			if ($objForm)
				return ($indicador4->TableVar == $objForm->GetValue("t"));
			if (@$_GET["t"] <> "")
				return ($indicador4->TableVar == $_GET["t"]);
		} else {
			return TRUE;
		}
	}
	...
	//  - open connection
	//
	function cindicador4_list() {
		global $conn;

		// Initialize table object
		$GLOBALS["indicador4"] = new cindicador4();
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'indicador4', TRUE);
	...
	//
	function Page_Init() {
		global $gsExport, $gsExportFile, $indicador4;
		global $Security;
	...
	$indicador4->Export = @$_GET["export"]; // Get export parameter
	$gsExport = $indicador4->Export; // Get export parameter, used in header
	$gsExportFile = $indicador4->TableVar; // Get export file, used in header
	if ($indicador4->Export == "print" || $indicador4->Export == "html") {

		// Printer friendly or Export to HTML, no action required
	}
	if ($indicador4->Export == "excel") {
	...
	//
	function Page_Main() {
		global $objForm, $gsSearchError, $Security, $indicador4;
		...
		if ($indicador4->getRecordsPerPage() <> "") {
			$this->lDisplayRecs = $indicador4->getRecordsPerPage(); // Restore from Session
		} else {
		...
		$indicador4->setSessionWhere($sFilter);
		$indicador4->CurrentFilter = "";

		// Export data only
		if (in_array($indicador4->Export, array("html","word","excel","xml","csv"))) {
			$this->ExportData();
	...
	function SetUpDisplayRecs() {
		...
		if ($sWrk <> "") {
			...
			$indicador4->setRecordsPerPage($this->lDisplayRecs); // Save to Session

			// Reset start position
			$this->lStartRec = 1;
			$indicador4->setStartRecordNumber($this->lStartRec);
		}
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			$indicador4->CurrentOrder = ew_StripSlashes(@$_GET["order"]);
			$indicador4->CurrentOrderType = @$_GET["ordertype"];
			$indicador4->UpdateSort($indicador4->idMer); // Field 
			$indicador4->setStartRecordNumber(1); // Reset start position
		}
	}
	...
	function LoadSortOrder() {
		global $indicador4;
		$sOrderBy = $indicador4->getSessionOrderBy(); // Get order by from Session
		if ($sOrderBy == "") {
			if ($indicador4->SqlOrderBy() <> "") {
				$sOrderBy = $indicador4->SqlOrderBy();
				$indicador4->setSessionOrderBy($sOrderBy);
			}
		}
	}
	...
	function SetUpStartRec() {
		...
		$this->lStartRec = $indicador4->getStartRecordNumber();
	...
	function LoadRecordset($offset = -1, $rowcnt = -1) {
		global $conn, $indicador4;

		// Call Recordset Selecting event
		$indicador4->Recordset_Selecting($indicador4->CurrentFilter);

		// Load list page SQL
		$sSql = $indicador4->SelectSQL();
		...
		$indicador4->Recordset_Selected($rs);
		return $rs;
	}

	// Load row based on key values
	function LoadRow() {
		global $conn, $Security, $indicador4;
		$sFilter = $indicador4->KeyFilter();

		// Call Row Selecting event
		$indicador4->Row_Selecting($sFilter);

		// Load sql based on filter
		$indicador4->CurrentFilter = $sFilter;
		$sSql = $indicador4->SQL();
	...
	function LoadRowValues(&$rs) {
		global $indicador4;
		$indicador4->idMer->setDbValue($rs->fields('idMer'));
	}
	...
	function RenderRow() {
		...
		$indicador4->Row_Rendering();

		// Common render codes for all row types
		// idMer

		$indicador4->idMer->CellCssStyle = "";
		$indicador4->idMer->CellCssClass = "";
		if ($indicador4->RowType == EW_ROWTYPE_VIEW) { // View row

			// idMer
			$indicador4->idMer->ViewValue = $indicador4->idMer->CurrentValue;
			$indicador4->idMer->CssStyle = "";
			$indicador4->idMer->CssClass = "";
			$indicador4->idMer->ViewCustomAttributes = "";

			// idMer
			$indicador4->idMer->HrefValue = "";
		}

		// Call Row Rendered event
		$indicador4->Row_Rendered();
	}
	...
	function ExportData() {
		...
		if ($indicador4->Export == "xml") {
			$XmlDoc = new cXMLDocument();
		} else {
			echo ew_ExportHeader($indicador4->Export);

			// Horizontal format, write header
			if ($sExportStyle <> "v" || $indicador4->Export == "csv") {
				$sExportStr = "";
				ew_ExportAddValue($sExportStr, 'idMer', $indicador4->Export);
				echo ew_ExportLine($sExportStr, $indicador4->Export);
			}
		}
		...
		while (!$rs->EOF && $this->lRecCnt < $this->lStopRec) {
			...
			if (intval($this->lRecCnt) >= intval($this->lStartRec)) {
				...
				$indicador4->RowType = EW_ROWTYPE_VIEW; // Render view
				$this->RenderRow();
				if ($indicador4->Export == "xml") {
					$XmlDoc->BeginRow();
					$XmlDoc->AddField('idMer', $indicador4->idMer->CurrentValue);
					$XmlDoc->EndRow();
				} else {
					if ($sExportStyle == "v" && $indicador4->Export <> "csv") { // Vertical format
						echo ew_ExportField('idMer', $indicador4->idMer->ExportValue($indicador4->Export, $indicador4->ExportOriginalValue), $indicador4->Export);
					}	else { // Horizontal format
						$sExportStr = "";
						ew_ExportAddValue($sExportStr, $indicador4->idMer->ExportValue($indicador4->Export, $indicador4->ExportOriginalValue), $indicador4->Export);
						echo ew_ExportLine($sExportStr, $indicador4->Export);
					}
				}
			}
		...
		if ($indicador4->Export == "xml") {
			...
			echo ew_ExportFooter($indicador4->Export);
		}
	}
