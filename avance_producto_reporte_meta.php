...
$query_metaReporte= "SELECT idMetaReporte,idMeta, idCronograma,archivo,detalle,(case estado when 1 then 'Corregir' when 2 then 'Revision' when 3 then 'Aprobado' end) as est, estado FROM meta_reporte WHERE idMeta=".$idMeta;
...
mysql_select_db($database_conexion, $conexion);
$query_metaReporteEstado= "SELECT estado FROM meta_reporte WHERE idMeta=$idMeta AND ( estado=2 OR estado=3)";
$mostrar_metaReporteEstado=mysql_query($query_metaReporteEstado, $conexion) or die(mysql_error());
$total_metaReporteEstado=mysql_num_rows($mostrar_metaReporteEstado);

mysql_select_db($database_conexion, $conexion);
$query_cronograma= "SELECT estado FROM cronograma WHERE idCronograma=$idCronograma";
...
<tr class="ewTableHeader">
    ...
    <td>Estado</td>
    <td align="right"><img src="images/close.png" onclick="func_cerrar_reporte_meta(<?php echo $idMeta?>)" style="cursor: pointer" title="Cerrar"></td>
</tr>
<?php
while($row_metaReporte=mysql_fetch_assoc($mostrar_metaReporte)){
mysql_select_db($database_conexion, $conexion);
$query_MRCnumero= "SELECT estado FROM meta_reporte_control WHERE idMetaReporte=".$row_metaReporte['idMetaReporte'];
$mostrar_MRCnumero=mysql_query($query_MRCnumero, $conexion) or die(mysql_error());
$total_MRCnumero=mysql_num_rows($mostrar_MRCnumero);
    ?>
<tr>
    <td><?php echo "<a href=\"files_consultoria/".utf8_encode($row_metaReporte['archivo'])."\" title='Extraer archivo'>".utf8_encode($row_metaReporte['archivo'])."</a>";?></td>
    <td><?php echo utf8_encode($row_metaReporte['detalle'])?></td>
    <td><?php echo $row_metaReporte['est']?></td>
    <td>
     <input type="button" name="revisar_meta" id="revisar_meta" value="Revisar" style="cursor: pointer" onclick="func_revisar_reporte_meta(<?php echo $row_metaReporte['idMetaReporte'];?>,<?php echo $idMeta;?>)">
     <?php if($row_metaReporte['estado']<2 && $Security->CanDelete() && $total_MRCnumero<1){?>
     <input type="button" name="borrar_meta" id="borrar_meta" value="Borrar" style="cursor: pointer" onclick="func_borrar_reporte_meta(<?php echo $row_metaReporte['idMetaReporte'];?>,'<?php echo utf8_encode($row_metaReporte['archivo']);?>',<?php echo $idMeta?>)">
     <?php }?>
    </td>
</tr>
<?php } ?>
<?php if($total_metaReporteEstado<1 && $Security->CanAdd() && $row_cronograma['estado']<2){?>
<tr>
    <td><input type="file" name="archivo_reporte_meta<?php echo $idMeta?>" id="archivo_reporte_meta<?php echo $idMeta?>" size="15"></td>
    <td><textarea name="detalle_reporte_meta<?php echo $idMeta?>" id="detalle_reporte_meta<?php echo $idMeta?>" rows="2" cols="30"></textarea></td>
    <td></td>
    <td>
        <input type="button" name="reporte_meta" value="Reportar" style="cursor: pointer" onclick="func_reporte_meta_join(<?php echo $idMeta?>,<?php echo $idCronograma?>)">
    </td>
</tr>
<?php }?>
